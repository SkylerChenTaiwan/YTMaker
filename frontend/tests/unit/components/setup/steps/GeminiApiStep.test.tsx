...
describe('GeminiApiStep', () => {
  ...
  it('should handle multiple consecutive tests', async () => {
    ...
    expect(mockSetApiKey).toHaveBeenCalledWith('gemini', 'key-2', true)
  })
  ...
  it('should complete full user flow', async () => {
    ...
    // 1. 輸入為空，blur 觸發驗證
    fireEvent.blur(input)
    let errorElements = screen.getAllByText(/請輸入.*Gemini.*API.*Key/i)
    expect(
    ...
    // 驗證最終 store 狀態
    expect(mockSetApiKey).toHaveBeenCalledWith('gemini', 'valid-key-123', true)
  })
})
